...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('subscription_status', sa.Enum('ACTIVE', 'CANCELLED', 'TRIAL', 'EXPIRED', name='subscriptionstatus'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('PENDING', 'ACCEPTED', 'DECLINED', 'EXPIRED', name='offerstatus'), nullable=False),
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('merchant_id', 'shopify_refund_id', name='uq_merchant_refund')
    )
    ...
    op.create_index(op.f('ix_offers_shopify_order_id'), 'offers', ['shopify_order_id'], unique=False)
    op.create_index(op.f('ix_offers_shopify_refund_id'), 'offers', ['shopify_refund_id'], unique=False)
    ...
    op.create_table(
        ...
        sa.Column('event_type', sa.Enum('CREATED', 'SENT', 'VIEWED', 'ACCEPTED', 'DECLINED', 'EXPIRED', 'CREDIT_ISSUED', name='eventtype'), nullable=False),
        sa.Column('metadata', sa.JSON(), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(['offer_id'], ['offers.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_offer_events_offer_id'), 'offer_events', ['offer_id'], unique=False)
